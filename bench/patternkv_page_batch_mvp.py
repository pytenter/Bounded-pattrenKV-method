...
def pack_mixed_v_pages(
    ...
    *,
    ...
    This is an experimental correctness-first ABI packer. It intentionally
    processes each request's precision mask independently and never reads
    `precision_mask[0]` as a batch-global layout.
    """
...
def patternkv_page_batched_v_decode(attn: torch.Tensor, cache: PatternKVPageBatchCache) -> torch.Tensor:
    """Experimental page-centric batched Value decode.

    This function is a single batched MVP operator at Python API level. It does
    not call the existing B=1 mixed-V kernel and does not materialize full
    historical V. It accumulates one logical page at a time.
    """
    ...
    for b in range(bsz):
        for page in range(num_pages):
            metadata_page = int(cache.metadata.metadata_page_table[b, page].item())
            valid = int(cache.metadata.valid_tokens[metadata_page].item())
            if valid <= 0:
                continue
            start = page * cache.page_size
            stop = start + valid
            v2_page_id = int(cache.metadata.v2_page_table[b, page].item())
            v4_page_id = int(cache.metadata.v4_page_table[b, page].item())
...
def selector_isolation_summary(precision_mask: torch.Tensor) -> dict[str, Any]:
    ...
    return {"selector_isolation_pass": len({row["selected_hash"] for row in rows}) == len(rows) if len(rows) > 1 else True, "requests": rows}
...
def cache_isolation_summary(
    ...
    *,
    ...
    base = patternkv_page_batched_v_decode(attn, base_cache)
    ...
    mutated = patternkv_page_batched_v_decode(attn, mutated_cache)
