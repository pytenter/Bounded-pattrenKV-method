...
def main() -> int:
    ...
    (OUT / "b1_semantic_reclosure.json").write_text(json.dumps(rec, indent=2, sort_keys=True) + "\n")
    ...
    (OUT / "b1_semantic_reclosure.md").write_text("# B1 Semantic Reclosure\n\n```json\n" + json.dumps(summary, indent=2, sort_keys=True) + "\n```\n")
    if rec.get("first_divergence"):
        (OUT / "first_divergence_trace_reclosure.json").write_text(json.dumps(rec["first_divergence"], indent=2, sort_keys=True) + "\n")
        (OUT / "first_divergence_trace_reclosure.md").write_text("# First Divergence Trace\n\n```json\n" + json.dumps(rec["first_divergence"], indent=2, sort_keys=True) + "\n```\n")
