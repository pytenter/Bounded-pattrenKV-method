...
class Qwen3PatternKVCache(Cache):
    def __init__(self, config: Any):
        ...
        self._seen_tokens = 0
        self.debug_stats: dict[int, dict[str, Any]] = {}
    ...
    def _initial_centroids(self, states: torch.Tensor, count: int) -> torch.Tensor | None:
        ...
        if flat.shape[1] >= k:
            ...
            centroids = flat.index_select(1, idx)
        else:
            repeat = (k + flat.shape[1] - 1) // flat.shape[1]
            centroids = flat.repeat(1, repeat, 1)[:, :k, :]
    ...
    def update(
        ...
        if bool(getattr(self.config, "patternkv_disable_quantization", False)):
        ...
        if cache is None:
            ...
            cache = build_cache_from_prefill(
                ...
                v_precision_selector=str(getattr(self.config, "patternkv_v_precision_selector", "base_v2")),
                v4_budget_fraction=float(getattr(self.config, "patternkv_v4_budget_fraction", 0.0)),
            ...
            # Prefill attention itself must be computed on exact current K/V.
        ...
        if full_k is None or full_v is None:
            raise RuntimeError(f"failed to reconstruct PatternKV cache for layer {layer_idx}")
        return full_k, full_v
    ...
    def reorder_cache(self, beam_idx: torch.LongTensor):
        raise NotImplementedError("Qwen3 PatternKV cache currently supports batch_size=1 sampling only")


class Qwen3Attention_PatternKV(Qwen3Attention):
    def forward(
        self,
        ...
        **kwargs: Any,
        ...
        if past_key_value is not None:
            if not isinstance(past_key_value, Qwen3PatternKVCache):
                raise TypeError(f"Qwen3 PatternKV requires Qwen3PatternKVCache, got {type(past_key_value)!r}")
...
class Qwen3Model_PatternKV(Qwen3Model):
    def __init__(self, config):
        super().__init__(config)
        for idx, layer in enumerate(self.layers):
            replacement = Qwen3Attention_PatternKV(config=config, layer_idx=idx)
            replacement.load_state_dict(layer.self_attn.state_dict(), strict=True)
            layer.self_attn = replacement


class Qwen3ForCausalLM_PatternKV(Qwen3ForCausalLM):
    def __init__(self, config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3Model_PatternKV(config)
    ...
    def forward(self, *args: Any, past_key_values: Optional[Cache] = None, use_cache: Optional[bool] = None, **kwargs: Any):
        ...
        if use_cache:
            ...
            if replace_cache:
                past_key_values = Qwen3PatternKVCache(self.config)
            elif not isinstance(past_key_values, Qwen3PatternKVCache):
                raise TypeError(f"Qwen3 PatternKV refuses non-native cache {type(past_key_values)!r}")
        return super().forward(*args, past_key_values=past_key_values, use_cache=use_cache, **kwargs)


def reset_qwen3_patternkv_runtime_state(model: nn.Module) -> None:
    return None


def collect_qwen3_patternkv_dynamic_stats(model: nn.Module, past_key_values: Any = None) -> dict[str, Any]:
    cache = past_key_values
    if not isinstance(cache, Qwen3PatternKVCache):
        return {"backend": "qwen3_patternkv", "layers": 0}
    if bool(getattr(cache.config, "patternkv_disable_quantization", False)):
        return {"backend": "qwen3_patternkv", "quantization_disabled": True, "layers": len(cache.full_key_cache)}
    stats = []
    for layer_cache in cache.layer_caches:
        stats.append(cache_segment_stats(layer_cache) if layer_cache is not None else {})
    return {"backend": "qwen3_patternkv", "layers": len(stats), "cache_segment_stats_per_layer": stats}
